  ...
  this.route('blog-posts',{ path: '/blog-posts/:user_id' }, function() {
    // this.route('index', { path: '/' });
        // *NOTE* Again, an 'index' route like this is generated for every route, but writing it out is not neccessary
          // If The user navigates to /blog-post, the current route will be blog-posts.index, and the posts/index template will be rendered into the {{outlet}} in the posts template. (If and index template does not exist, it will look for blog-post.hbs I am assuming...)
    this.route('new');
  });
  this.route('signup');
  this.route('login');
